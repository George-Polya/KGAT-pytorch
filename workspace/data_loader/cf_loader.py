...
class _CustomDatasetFetcher(object):
    ...
    def fetch(self, possible_batched_index):
        ...
        user_dict = self.dataset.user_dict[self.dataset.phase]
        return self.dataset.loaderbase.generate_cf_batch(user_dict, batch_size)

class CF_DataLoader(data.DataLoader):
    def __init__(self, dataset, batch_size, shuffle, collate_fn, pin_memory):
        super(CF_DataLoader, self).__init__(
